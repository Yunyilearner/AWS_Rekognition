...
'''
#啟用(零號)相機
cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)  #參數 cv2.CAP_DSHOW 是微軟特有的，用來防止釋放攝影鏡頭時的[ WARN:0] terminating async callback警告

milli = int(round(time.time() * 1000))

while(True):    #while 迴圈 在這裡一直都是 true 不會跳離開，會永遠亮著開啟
  milli = 2*1000   # 2*1000 毫秒
  ...
  cv2.imwrite('suspicious.jpg', frame)  #照到的影像要存成什麼檔名
...
with open('C:/Users/yuniy/Desktop/New pioneers in the industry/Python-Programming Language/AWS_Rekognition/accessKeys.csv','r') as input:
    next(input)
...
#RequiredEquipmentTypes : 指定人員要穿戴的 PPE 類型到 summaryAttributes， RequiredEquipmentTypes 的值為 list:[ FACE_COVER | HAND_COVER | HEAD_COVER]
    ...
    suspicious_person = response["Summary"]["PersonsWithRequiredEquipment"]# 三個部位都有遮掩的人被判定為可疑人物，回傳符合指定的 Attributes 的人員 ID
    ...
    for ppelabl in ppeitemlabel:            
        PPEinfo = str(ppelabl["Type"]) + '\n\tCovers body part: ' + str(ppelabl ["CoversBodyPart"]["Value"]) + '\n\tConfidence: ' + '\n\t' + str(ppelabl["CoversBodyPart"]["Confidence"])

    return person_count, num_of_suspicious_person,suspiciousperson, PPEinfo  
# 回傳 person_count 給 main() 涵式計算總人數;回傳 num_of_suspicious_person 給 main() 涵式計算可疑人物總人數; 回傳 suspiciousperson、PPEinfo值給 main() 涵式顯示 GUI 資訊


'''
-------------------------------------------------------------------------------------------
Part 4: 顯示圖片並框出可疑人物與遮掩標的   (executable)                           
-------------------------------------------------------------------------------------------
'''
...
def detect_ppe(photo, confidence):   
    ...
    response = client.detect_protective_equipment(Image={'Bytes': image_binary}, 
        SummarizationAttributes={'MinConfidence':50, 'RequiredEquipmentTypes':['HEAD_COVER', 'FACE_COVER', 'HAND_COVER']})
...
def detect_labels(photo): 
    ...
    return len(response['Labels']), found_robber, weaponinfo


'''
-------------------------------------------------------------------------------------------
Part 6: Detect Moderating content(內容管制) 偵測影像中不適當內容、冒犯性內容-暴力
-------------------------------------------------------------------------------------------
'''
...
def moderating_content(photo):
    ...
    response = client.detect_moderation_labels(Image={'Bytes': imgbytes}) #呼叫API 
    ...
    for label in response['ModerationLabels']: #分析影像中不當內容的類別標籤
    ...
    moderatinginfo = "系統判斷影像出現冒犯性內容:" + '\n' + str(ModerationLabelvalue)
    #Detect Moderating content 偵測到影像中出現什麼類型的冒犯性內容
...
#建立 Frame (把元件變成群組的容器)，區隔主視窗空間的區塊
    ...
    #函式(define_layout)用來定義區塊縮放時比例，引入 obj 為 UI widget、cols 該 widget 中有幾欄、row 該 widget 中有幾列
    ...
    img = Image.open('C:/Users/yuniy/Desktop/New pioneers in the industry/Python-Programming Language/Python code and GUI course/Gun_1 analysis.jpg')
    ...
    label3 = tk.Label(frame2, text = suspiciousperson, bg='white', fg='orange',font=('標楷體', 18),wraplength = 500, justify = 'left', anchor = 'w')
    label4 = tk.Label(frame2, text = PPEinfo, bg='white', fg='orange',font=('Times New Romance', 18),wraplength = 500, justify = 'left', anchor = 'w')
...
-------------------------------------------------------------------------------------------
'''
def GUI_Robber(weaponinfo, moderatinginfo):
    ...
    # sticky 是對齊方式，這邊用 align_mode 來統一所有的對其方式，而給予字串 ‘nswe’ 是置中的意思 ( n 上 s 下 w左 e 右)：
    frame1 .grid(column=0, row=0, padx=pad, pady=pad, rowspan=2, sticky=align_mode)
    frame2 .grid(column=1, row=0, padx=pad, pady=pad, sticky=align_mode)
    
    
    def define_layout(obj, cols=1, rows=1):  #obj = window
    ...
    label2= tk.Label(frame2, text = "資訊欄 :", bg='white', fg='black',font=('標楷體', 18), anchor = 'w')
    label3= tk.Label(frame2, text = weaponinfo, bg='white', fg='orange',font=('標楷體', 18),wraplength = 500, justify = 'left', anchor = 'w') 
    
    label4 = tk.Label(frame2, text = moderatinginfo, bg='white', fg='orange',font=('標楷體', 18),wraplength = 500, justify = 'left', anchor = 'w')
